...
async function seedShopIfEmpty(shopId: number) {
  ...
  for (const st of demo.staff) {
    ...
    if (newId != null) allStaffIds.push(String(newId));
  }
...
// ─── Stripeクライアント（env var優先、なければReplitコネクター） ───
...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.use(async (_req, _res, next) => {
    try { await initBookingTables(); } catch (e) {}
    next();
  });
  ...
  app.get("/api/areas", async (_req, res) => {
    ...
    catch (e: any) { res.status(500).json({ message: "Failed to fetch areas" }); }
  });

  // ─── カテゴリ ───
  app.get("/api/categories", async (_req, res) => {
    try { res.json(await sql`SELECT * FROM categories ORDER BY id`); }
    catch (e: any) { res.status(500).json({ message: "Failed to fetch categories" }); }
  });
  ...
  app.get("/api/shops", async (req, res) => {
    ...
    } catch (e: any) { res.status(500).json({ message: "Failed to fetch shops" }); }
  });
  ...
  app.get("/api/shops/:shopId/staff", async (req, res) => {
    ...
    try {
      ...
      const courses = await safeQuery(() => sql`SELECT COUNT(*) as cnt FROM booking_courses WHERE shop_id = ${shopId} AND is_active = true`);
      // コース一覧を別途取得（COUNT後に実クエリ）
      ...
      res.json(rows.map((s: any) => ({
        ...
        courseIds: courseRows.filter((c: any) => (c.staff_ids || []).includes(s.id.toString())).map((c: any) => c.id.toString()),
      })));
  ...
  app.get("/api/shops/:shopId/slots", async (req, res) => {
    ...
    try {
      ...
      let tableCount = 1;
      if (parseInt(settingsCnt[0]?.cnt || "0") > 0) {
        const settingsRows = await sql`SELECT table_count FROM booking_settings WHERE shop_id=${shopId}`;
        tableCount = Math.max(parseInt(settingsRows[0]?.table_count || "1", 10) || 1, 1);
      }

      let courseDuration = 30;
      if (courseId) {
        const courseCnt = await sql`SELECT COUNT(*) as cnt FROM booking_courses WHERE id=${courseId} AND shop_id=${shopId}`;
        if (parseInt(courseCnt[0]?.cnt || "0") > 0) {
      ...
      const reservations = await safeQuery(() => sql`
      ...
      for (const r of reservations) {
        ...
        for (const slot of allSlots) {
          ...
          if (rStartMin < sStartMin + 30 && rEndMin > sStartMin) slotCount.set(slot, (slotCount.get(slot) || 0) + 1);
        }
      }
      ...
      const result = allSlots.map(slot => {
        const [sh2, sm2] = slot.split(":").map(Number);
        const sStartMin = sh2 * 60 + sm2;
        if (sStartMin + courseDuration > 19 * 60) return { time: slot, available: false };
        let maxConcurrent = 0;
  ...
  app.put("/api/shops/:shopId/settings", async (req, res) => {
    ...
    try {
      ...
      await sql`INSERT INTO booking_settings (shop_id, store_name, store_description, store_address, store_phone, store_email, store_hours, store_closed_days, banner_url, staff_selection_enabled, table_count, max_party_size, updated_at)
        VALUES (${shopId}, ${s.store_name||""}, ${s.store_description||""}, ${s.store_address||""}, ${s.store_phone||""}, ${s.store_email||""}, ${s.store_hours||""}, ${s.store_closed_days||""}, ${s.banner_url||""}, ${s.staff_selection_enabled||"false"}, ${tc}, ${mp}, NOW())
  ...
  app.get("/api/shops/:shopId/reservations", async (req, res) => {
    ...
    if (isNaN(shopId)) return res.status(400).json({ message: "Invalid shop ID" });
    try {
      const rows = await safeQuery(() => sql`SELECT * FROM booking_reservations WHERE shop_id=${shopId} ORDER BY date DESC, time DESC`);
  ...
  app.get("/api/stripe/config", async (_req, res) => {
    try { res.json({ publishableKey: await getStripePublishableKeyValue() }); }
    catch { res.json({ publishableKey: "" }); }
  });
  ...
  app.get("/api/stripe/connect/status/:shopId", async (req, res) => {
    try {
      ...
      const shop = rows[0];
      if (!shop.stripe_connect_id) return res.json({ shopId, accountId: null, status: "none", connected: false });
      try {
        ...
        return res.json({ shopId, accountId: shop.stripe_connect_id, status: shop.stripe_connect_status || "pending", connected: shop.stripe_connect_status === "active" });
      }
  ...
  app.post("/api/stripe/connect/payment-intent", async (req, res) => {
    try {
      ...
      if (!rows.length) return res.status(404).json({ error: "Shop not found" });
      const shop = rows[0];
      if (!shop.stripe_connect_id) return res.status(400).json({ error: "この店舗はStripe Connect未設定です" });
      const stripe = await getStripeClient();
      let pi;
      try {
        ...
        console.warn("Stripe Connect fallback:", connectErr.code);
        pi = await stripe.paymentIntents.create({ amount: Math.round(amount), currency: "jpy", payment_method_types: ["card"], description: courseName || "コース予約", metadata: { shop_id: String(shopId), shop_name: shop.name, course_id: courseId || "", course_name: courseName || "", note: "pending_onboarding_fallback" } });
      }
  ...
  app.post("/api/stripe/connect/onboard/:shopId", async (req, res) => {
    try {
      ...
      if (!accountId) {
        ...
        await stripe.accounts.update(accountId, { capabilities: { card_payments: { requested: true }, transfers: { requested: true } } });
      }
  ...
  app.post("/api/fix-subcategory-slugs", async (_req, res) => {
    try {
      ...
      for (const shop of shops) {
        ...
        if (!current || current === "null") { await sql`UPDATE shops SET subcategory = NULL WHERE id = ${shop.id}`; updated++; continue; }
        const newSub = subcatMap[current];
        if (newSub && newSub !== current) { await sql`UPDATE shops SET subcategory = ${newSub} WHERE id = ${shop.id}`; updated++; }
      }
  ...
  app.post("/api/fix-shop-slugs", async (_req, res) => {
    try {
      ...
      for (const shop of shops) {
        ...
        if (newArea !== shop.area || newCat !== shop.category) { await sql`UPDATE shops SET area = ${newArea}, category = ${newCat} WHERE id = ${shop.id}`; updated++; }
      }
  ...
  app.post("/api/seed-shops", async (_req, res) => {
    try {
      ...
      res.json({ ok: true, total: parseInt(total[0].cnt), results, message: "Use /api/seed-test for individual shop seeding" });
